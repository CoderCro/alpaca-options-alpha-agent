...
**Risk guardrails** (`src/guardrails.py` -- the actual security boundary for both companies, enforced in code, not just a prompt instruction): a kill switch (checked first, always), the blackout calendar, watchlist membership, defined-risk structure only (single-leg long calls/puts -- vertical spreads deferred, see Known Limitations), a 3%-of-equity per-trade risk cap (6% for Company C specifically, via `PER_TRADE_RISK_PCT_OVERRIDES` -- its delta-hedge leg is structurally larger in dollar terms than A/B's single-leg premium risk, see "Company C" below), a max of 8 concurrent positions, a 25%-of-equity aggregate risk cap, a 15/day new-entry cap, and a daily-loss circuit breaker. Exits and trims always bypass every cap except the kill switch, so neither company can be blocked from cutting a loss by its own risk limits.
...
- **Sized by the hedge, not just the premium, and biased away from ATM.** A cheap short-DTE put's own premium is a poor guide to its total risk: the delta-hedge leg (shares x spot price) is routinely far larger in dollar terms -- live-confirmed on DIS, a $2,987 put needed a $179,373 hedge, 60x the per-trade cap. `company_c_agent.py` now sizes to `floor(min(premium-derived qty, hedge-derived qty))`, skipping cleanly (`hedge_unaffordable`) rather than forcing a doomed trade when even one contract can't be hedged affordably. Since near-ATM strikes run delta ~0.5-0.6 (making the hedge disproportionately expensive at $100+ spot prices), Company C also picks further-OTM puts (`vol_edge.MONEYNESS_RANGE = (0.97, 0.995)`, live-calibrated -- delta is extremely strike-sensitive at this DTE), skips underlyings priced above `vol_edge.MAX_UNDERLYING_PRICE` (170, derived from the cap and a representative delta) before ever fetching their chain, and uses its own, higher per-trade risk cap (`guardrails.PER_TRADE_RISK_PCT_OVERRIDES = {"c": 6.0}` vs. A/B's shared 3%) to reflect that structurally larger hedge notional. Net effect: Company C's effectively-active universe on a given day is whichever handful of the 29 watchlist names sit under that price, not all 29 -- fewer names trade, but the ones that do are real, affordable trades rather than always-doomed attempts.
...
- **Post-mortem, Sep 15-18: a degenerate LLM response could crash Company A's entire process instead of failing closed.** `featherless_review._parse_verdict`'s regex fallback (for when the model doesn't return clean JSON) re-ran `json.loads` on the matched `"{...}"` substring *outside* any try/except -- live-confirmed when the model produced a garbled, partly non-English response containing a `{...}`-shaped fragment that still wasn't valid JSON, raising an uncaught `JSONDecodeError` that killed Company A's run (traceback, full process exit) rather than vetoing the trade as the "always fail closed" design promises. Separately, Company B's LangChain agent degenerated into an unbounded repetition loop on its final-turn answer ("Based!!!!!!..." repeated thousands of times) -- no tool calls that cycle, so no order was ever at risk, but the raw text was logged verbatim with no length cap. Fixed: `_parse_verdict`'s second `json.loads` now fails closed on bad JSON like the first one; `max_tokens` added to both the veto call and Company B's reasoning call so a degenerate response can't run away; `scheduler.py` now truncates any subprocess output over 500 chars before logging it. 2 new/updated tests, 222/222 passing.
- **Post-mortem, Sep 2-4: the hedge leg was failing far more than "occasionally"**. `hedge_limit_price` was passed straight from `signal["spot"]` / a live position's `current_price` -- a raw, multi-decimal value (e.g. `108.605`) -- into a stock limit order, which Alpaca rejects outright ("sub-penny increment does not fulfill minimum pricing criteria"). Live audit trail: 5 of 8 entry attempts on Sep 2 hit this, each leaving a filled, unhedged put -- pure directional exposure the strategy is designed to avoid. Those naked puts sat exposed until they closed/expired around Sep 3-4; this time the account came out ahead (+~$670 on the naked exposure vs. +~$37 on the 3 properly-hedged round trips), but that was luck, not the vol-edge working -- an adverse move would have hit the account with the very risk delta-hedging exists to remove. Fixed (`company_c_agent.py`: both `hedge_limit_price` call sites now `round(..., 2)` before submission); the underlying two-call non-atomicity above is unchanged. 2 new regression tests, 221/221 passing. Found after the trading window had already closed for the hackathon -- documented rather than silently left as a footnote, since it's the most material risk to running this strategy further.
